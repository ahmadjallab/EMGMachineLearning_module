...
import pywt
from EMGPatternRecognition.ClassificationUtility import Dissolves_array_reshape
...
def features_estimation(signal, channel_name, fs, frame, step, plot=False):
    ...
    if plot:
        plot_features(signal, channel_name, fs, total_feature_matrixpd, step)
...
def Handel_timeD_featuresEngeenring_withReshape (filter_emgMatrix, channels_name, fs, frame, step, plot=False,extraction_features=[]) :
    '''
    ...
    *for frame and step range(frame, signal.size, step) signal.size this for each channel
    ...
    for channel in filter_emgMatrix:
        total_feature_matrix, _ , time_matrix,_ = features_estimation(channel,channels_name[counter], fs,frame, step,plot=False)
        counter += 1
